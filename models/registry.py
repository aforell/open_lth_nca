...
from models import cifar_resnet, cifar_vgg, mnist_lenet, imagenet_resnet, mnist_simplenca, mnist_nca, nca_3d, nca, med_nca
from models import bn_initializers, initializers
from platforms.platform import get_platform

registered_models = [mnist_lenet.Model, cifar_resnet.Model, cifar_vgg.Model, imagenet_resnet.Model, mnist_simplenca.Model, mnist_nca.Model, nca_3d.Model, nca.Model, med_nca.Model]
...
def get(model_hparams: ModelHparams, outputs=None):
    ...
    for registered_model in registered_models:
        if registered_model.is_valid_model_name(model_hparams.model_name):
            model = registered_model.get_model_from_name(model_hparams.model_name, init_fn, outputs, model_hparams)
            break
